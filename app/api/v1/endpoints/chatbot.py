...
@router.put("/edit/{chatbot_id}", status_code=status.HTTP_200_OK, response_model=ChatBotOut)
def update(token: str, chatbot_id: str, chatbot_update: ChatBotUpdate, db: Session = Depends(deps.get_db)) -> ChatBotOut:
    existing_chatbot = chatbot_service.get(db, token= token, chatbot_id = chatbot_id)
    if not existing_chatbot:
        raise HTTPException(status_code=404, detail="Chatbot not found")
    updated_chatbot = crud_chatbot.update(db, db_obj=existing_chatbot, obj_in=chatbot_update)
    return updated_chatbot


@router.get("/get/{chatbot_id}", status_code=status.HTTP_200_OK, response_model=ChatBotOut)
